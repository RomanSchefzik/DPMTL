################################################################################
#' @title Solver of MTL with logistic loss and low-rank structure
#' @description Solver of MTL with logistic loss and low-rank structure
...
#' @details Solver of MTL with logistic loss and low-rank structure. \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
...
#' matrix(rnorm(200000),nrow=400,ncol=500))
#' set.seed(24)
...
#' model.nonprivate<-LR_MTL_Trace(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts0)
#' #example for a private model
#' opts1<-list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=0.7,nRunsSensitAn=100)
#' set.seed(24)
#' model.private<-LR_MTL_Trace(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts1)
#' 
#' @export 
LR_MTL_Trace <- function (X, Y, lam, C, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100)){
  ...
  LR_iter_update_tasks <- function(W){
    ...
    maxGradWsDifference <- simulateDifferences_LR(rData, C, W, X, Y,nSimulations=opts$nRunsSensitAn)
  ...
  LR_funcVal_eval_tasks <- function (W){
    ...
    maxDifference <- computeMaxDifference_LR(X, Y, W, C, funcVal,nSimulations=opts$nRunsSensitAn)
