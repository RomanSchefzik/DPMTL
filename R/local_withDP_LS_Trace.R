################################################################################
#' @title Solver of MTL with least-square loss and low-rank structure
#' @description Solver of MTL with least-square loss and low-rank structure
#' @param X A set of feature matrices
#' @param Y A set of response vectors
...
#' @details Solver of MTL with least-square loss and low-rank structure. \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
...
#' matrix(rnorm(100000),nrow=200,ncol=500))
#' set.seed(24)
#' Y0<-list(rnorm(200),rnorm(300),rnorm(200))
#' lam0<-0.05
#' C0<-1
#' #non-private model
#' opts0<-list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=NULL)
#' model.nonprivate<-LS_MTL_Trace(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts0)
#' #example for a private model
#' opts1<-list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=0.7,nRunsSensitAn=100)
#' set.seed(24)
#' model.private<-LS_MTL_Trace(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts1)
#' 
#' @export  
LS_MTL_Trace <- function (X, Y, lam, C, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100)){
  ...
  LS_iter_update_tasks <- function(W){
    ...
    maxGradWsDifference <- simulateDifferences_LS(rData, C, W, X, Y,nSimulations=opts$nRunsSensitAn)
  ...
  LS_funcVal_eval_tasks <- function ( W){
    ...
    #return(funcVal)
    maxDifference <- computeMaxDifference_LS(X, Y, W, C, funcVal,nSimulations=opts$nRunsSensitAn)
    ...
    }else{funcVal<-sum(sapply(1:nTasks, function(k){mean(0.5 * (Y[[k]] - X[[k]] %*% W[,k])^2)})) + C * norm(W, 'f')^2}
    return(funcVal)
