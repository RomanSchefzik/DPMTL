################################################################################
#' @title Solver of MTL with least-square loss and joint feature selection
#' @description Solver of MTL with least-square loss and joint feature selection
...
#' @details Solver of MTL with least-square loss for joint feature selection. \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
...
#' model.nonprivate<-LS_MTL_L21(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts0)
#' #example for a private model
#' opts1<-list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=0.7,nRunsSensitAn=100)
#' set.seed(24)
#' model.private<-LS_MTL_L21(X=X0,Y=Y0,lam=lam0,C=C0,opts=opts1)
#' 
#' @export  
LS_MTL_L21 <- function (X, Y, lam, C, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100)){
