################################################################################
#' @title Solver of MTL with least-square loss and network incorporation
#' @description Solver of MTL with least-square loss and network incorporation
...
#' @details Solver of MTL with least-square loss and network incorporation. \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
...
#' model.nonprivate<-LS_MTL_Net(X=X0,Y=Y0,lam=lam0,C=C0,G=G0,opts=opts0)
#' #example for a private model
#' opts1<-list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=0.7,nRunsSensitAn=100)
#' set.seed(24)
#' model.private<-LS_MTL_Net(X=X0,Y=Y0,lam=lam0,C=C0,G=G0,opts=opts1)
#' 
#' @export 
LS_MTL_Net<- function (X, Y, lam, C, G, opts){
  ...
  LS_iter_update_tasks <- function(W){
    ...
    maxGradWsDifference <- simulateDifferences_LS_Net(rData, C, W, X, Y, G,nSimulations=opts$nRunsSensitAn)
  ...
  LS_funcVal_eval_tasks <- function ( W){
    ...
    funcVal <- sum(sapply(1:nTasks, function(k){mean(0.5 * (Y[[k]] - X[[k]] %*% W[,k])^2)})) +  C * norm(W%*%G, 'f')^2
    #return(funcVal)
    maxDifference <- computeMaxDifference_LS_Net(X, Y, W, C, funcVal,G,nSimulations=opts$nRunsSensitAn)
    ...
    }else{funcVal<-sum(sapply(1:nTasks, function(k){mean(0.5 * (Y[[k]] - X[[k]] %*% W[,k])^2)})) +  C * norm(W%*%G, 'f')^2}
    return(funcVal)
